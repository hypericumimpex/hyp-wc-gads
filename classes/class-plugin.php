...
class Plugin {
	...
	public function bootstrap() {
		try {
			load_plugin_textdomain( 'woocommerce-google-ads', false, KK_WC_PLUGIN_REL_PATH . '/languages' );

			if ( $this->_bootstrapped ) {
				throw new \Exception( __( 'Google Ads for WooCommerce plugin can only be called once', 'woocommerce-google-ads' ), self::ALREADY_BOOTSTRAPED );
			}
	...
	public function plugin_action_links( $links ) {
		...
		if ( function_exists( 'wc' ) ) {
			$plugin_links[] = '<a href="' . esc_url( Helper::get_plugin_page() ) . '">' . __( 'Dashboard', 'woocommerce-google-ads' ) . '</a>';
		}

		$plugin_links[] = '<a href="https://kliken.com/contact/">' . __( 'Support', 'woocommerce-google-ads' ) . '</a>';
	...
	/**
	 * Action to initialize plugin's WooCommerce Integration.
	 *
	...
	public function admin_enqueue_scripts() {
		...
		wp_localize_script(
			...
			[
				...
				'confirmMessage' => esc_html__( 'This will dismiss the message permanently. Are you sure?', 'woocommerce-google-ads' ),
			]
		);
	}
	...
	private function check_dependencies() {
		if ( ! function_exists( 'wc' ) ) {
			throw new \Exception( __( 'Google Ads for WooCommerce requires WooCommerce to be activated', 'woocommerce-google-ads' ), self::DEPENDENCIES_UNSATISFIED );
		}
		...
		if ( version_compare( wc()->version, $required_woo_version, '<' ) ) {
			...
			throw new \Exception( sprintf( __( 'Google Ads for WooCommerce requires WooCommerce version %s or greater', 'woocommerce-google-ads' ), $required_woo_version ), self::DEPENDENCIES_UNSATISFIED );
		}
	}
	...
	private function run() {
		add_action( 'woocommerce_integrations', [ $this, 'woocommerce_integrations' ] );
