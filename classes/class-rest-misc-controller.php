...
class REST_Misc_Controller extends \WC_REST_CRUD_Controller {
	...
	public function save_google_token( $request ) {
		...
		if ( ! $token ) {
			return new \WP_Error( 'rest_bad_request', __( 'Invalid Data', 'woocommerce-google-ads' ), [ 'status' => 400 ] );
		}
	...
	public function get_wc_services_shipping_methods( $request ) {
		...
		if ( ! $data || empty( $data ) || empty( $data->services ) ) {
			return new \WP_Error( 'no_wc_services', __( 'Invalid Data', 'woocommerce-google-ads' ), [ 'status' => 404 ] );
		}
	...
	public function get_shipping_methods_permissions_check( $request ) {
		if ( ! wc_rest_check_manager_permissions( 'shipping_methods', 'read' ) ) {
			return new \WP_Error( 'woocommerce_rest_cannot_view', __( 'Sorry, you cannot list resources.', 'woocommerce-google-ads' ), [ 'status' => rest_authorization_required_code() ] );
		}
		return true;
	}
}
