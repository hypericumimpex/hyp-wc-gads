...
class Helper {
	...
	public static function wc_log( $level, $message, $force = false ) {
		if ( ( WP_DEBUG || $force )
			...
			wc_get_logger()->log( $level, $message, [ 'source' => 'woocommerce-google-ads' ] );
		}
	}
	...
	public static function build_authorization_url( $account_id, $application_token ) {
		...
		$authorization_url = get_site_url() . '/wc-auth/v1/authorize'
			...
			. '&scope=read_write'
			. '&user_id=' . base64_encode( $account_id . ':' . $application_token . ':' . base64_encode( get_site_url() ) )
			. '&return_url=' . rawurlencode( KK_WC_WOOKLIKEN_BASE_URL . 'smb/' )
	...
	public static function get_onboarding_message() {
		return sprintf(
			wp_kses(
				...
				__( '<strong>Google Ads for WooCommerce plugin is almost ready.</strong> <a href="%s">Click here</a> to get started.', 'woocommerce-google-ads' ),
				[
					'strong' => [],
					'a'      => [ 'href' => [] ],
				]
